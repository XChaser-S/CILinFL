...
import os
from CIL_train_test.path_adaptive import new_model_dir_path
from agents.imitation.path_adaptive import model_dir_path
import scipy
...
class ImitationLearning(Agent):
    ...
    def __init__(self, city_name, avoid_stopping, memory_fraction=0.25, image_cut=[115, 510]):
        ...
        with tf.name_scope("Network"):
            self._network_tensor = load_imitation_learning_network(self._input_images,
                                                                   self._input_data,
                                                                   self._image_size, self._dout, self._mode)

        # import os
        # dir_path = os.path.dirname(__file__)

        # self._models_path = model_dir_path + '/model/'
        # self._models_path = new_model_dir_path + '/log5/log5'
        self._models_path = r'D:\soft\Desktop\imitation_learning_master\imitation_learning_master\log1'
    ...
    def _compute_action(self, rgb_image, speed, direction=None):
        ...
        #                                               self._image_size[1]])
        image_input = resize(rgb_image, output_shape=[self._image_size[0],
                                                      self._image_size[1]])
    ...
    def _control_function(self, image_input, speed, control_input, sess):
        ...
        if self._avoid_stopping:
            ...
            print(f'real_speed:{real_speed}m/s')
            print(f'real_predicted:{real_predicted}m/s')
